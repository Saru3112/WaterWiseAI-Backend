...
const damCoordinates = {
  ...
  'sengulam (pumping storage dam)': { latitude: 10.0108, longitude: 77.0325 },
};
    ...
    $('table tr').slice(2, 20).each((index, row) => {
      ...
      if (columns.length > 21) {
        ...
        const dam = {
          ...
          latitude: damCoordinates[damKey]?.latitude || null,
          longitude: damCoordinates[damKey]?.longitude || null,
          data: [{
...
async function fetchDamDetails() {
  try {
    ...
    for (const newDam of dams) {
      ...
      if (existingDam) {
        ...
        if (!dateExists) {
          ...
          Object.assign(existingDam, { ...newDam, data: existingDam.data });
          dataChanged = true;
        }
      } else {
    ...
    if (dataChanged) {
      ...
      console.log('No new updates. live.json remains unchanged.');
    }
